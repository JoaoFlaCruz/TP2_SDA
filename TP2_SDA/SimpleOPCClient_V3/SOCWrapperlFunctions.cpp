...
//
// Contudo, estes comendos "sprintf" devem ser adaptados às características
...
bool VarToStr(VARIANT pvar, char* buffer)
{
	...
	case VT_BOOL:
	case VT_I1:
		sprintf_s(buffer, 128, "%d", pvar.iVal);	break;
	case VT_I2:
		sprintf_s(buffer, 128, "%d", pvar.intVal);	break;
	case VT_I4:
		sprintf_s(buffer, 128, "%ld", pvar.intVal);	break;
	case VT_UI1:
		sprintf_s(buffer, 128, "%06u", pvar.uiVal);	break;
	case VT_UI2:
		sprintf_s(buffer, 128, "%u", pvar.ulVal);	break;
	case VT_UI4:
		sprintf_s(buffer, 128, "%lu", pvar.ulVal);	break;
	case VT_R4:
		sprintf_s(buffer, 128, "%6.2f", pvar.fltVal);	break;
	case VT_R8:
		sprintf_s(buffer, 128, "%lf", pvar.dblVal);	break;
	case VT_BSTR:
		sprintf_s(buffer, 128, "%ls", pvar.bstrVal);	break;
	default:
